...
class ActorRolloutRefWorker(MegatronWorker, DistProfilerExtension):
    ...
    def _maybe_export_topk_actor(self, checkpoint_path: str, global_step: int, val_metric: float):
        import os, json, shutil, subprocess, sys, socket
        import torch
        ...
        # ==== 仅 rank0 ====
        try:
            assert torch.cuda.is_available(), "[topk] CUDA not available but GPU is required."
            K = int(getattr(self.config, "max_actor_model_to_keep", 0))
            if K <= 0 or val_metric is None:
                _log(f"[topk] skip export. K={K}, metric={val_metric}")
                return  # 也要让其他 rank 继续
            actor_src = checkpoint_path
            if not os.path.isdir(actor_src):
                _log(f"[topk] actor src not found: {actor_src}")
                return

            # 统一放到 global_step_x 的上一层
            exp_root = os.path.abspath(os.path.join(checkpoint_path, "..", ".."))
            index_path = os.path.join(exp_root, "best_actor_index.json")
            export_root = os.path.join(exp_root, "best_actor")
            ...
            # 是否能进 Top-K（越大越好）
            need = len(topk) < K or float(val_metric) > float(min(topk, key=lambda x: x["metric"])["metric"])
            if not need:
                _log(f"[topk] metric {val_metric:.6f} not in top-{K}, skip.")
                return

            export_dir = os.path.join(export_root, f"gs{int(global_step)}_m{float(val_metric):.6f}")
            ...
            actor_src_abs  = os.path.abspath(checkpoint_path)   # 就是 actor 目录
            export_dir_abs = os.path.abspath(export_dir)

            cmd = [
                sys.executable, "-m", "verl.model_merger", "merge",
                "--backend", "megatron",
                "--local_dir", actor_src_abs,
                "--target_dir", export_dir_abs,
                "--tie-word-embedding",
            ]

            # 为子进程设置“独立”的单进程分布式环境（注意：用覆盖而不是 setdefault）
            env = os.environ.copy()
            env["MASTER_ADDR"] = "127.0.0.1"
            env["MASTER_PORT"] = str(_pick_free_port())   # 任选空闲端口
            env["WORLD_SIZE"]  = "1"
            env["RANK"]        = "0"
            env["LOCAL_RANK"]  = "0"
            # 稳妥（可选）
            env["NCCL_ASYNC_ERROR_HANDLING"] = env.get("NCCL_ASYNC_ERROR_HANDLING", "1")
            env["TORCH_NCCL_BLOCKING_WAIT"]  = env.get("TORCH_NCCL_BLOCKING_WAIT",  "1")
            # 避免 torchrun/elastic 残留变量干扰（可选）
            for k in ["TORCHELASTIC_RUN_ID", "RDZV_ENDPOINT", "RDZV_ID", "GROUP_RANK",
                    "LOCAL_WORLD_SIZE", "ROLE_RANK", "ROLE_NAME"]:
                env.pop(k, None)

            _log(f"[topk] exporting actor -> {export_dir_abs}")
            ret = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, env=env)
            if ret.returncode != 0:
                _log(f"[topk][ERROR] merger rc={ret.returncode}\n{ret.stdout[:2000]}")
                shutil.rmtree(export_dir_abs, ignore_errors=True)
                return

            # 更新索引（按 metric 降序），并清理多余
            topk.append({"metric": float(val_metric), "step": int(global_step), "export_dir": export_dir})
            topk.sort(key=lambda x: x["metric"], reverse=True)
            while len(topk) > K:
                old = topk.pop(-1)
                if old.get("export_dir") and os.path.isdir(old["export_dir"]):
                    _log(f"[topk] remove {old['export_dir']}")
                    shutil.rmtree(old["export_dir"], ignore_errors=True)

            with open(index_path, "w") as f:
                json.dump(topk, f, indent=2)
            _log(f"[topk] kept {len(topk)}/{K}, best={topk[0]['metric']:.6f}")
        finally:
            if torch.distributed.is_initialized():
                torch.distributed.barrier()

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def save_checkpoint(self, checkpoint_path, hdfs_path=None, global_step=0, max_ckpt_to_keep=None, val_metric=None):
        if self._is_offload_param:
            load_megatron_model_to_gpu(self.actor_module)
        ...
        if self._is_offload_param:
            offload_megatron_model_to_cpu(self.actor_module)
        # 额外存储 topk actor 模型
        self._maybe_export_topk_actor(checkpoint_path, global_step, val_metric)
