'''
VLLM_RL_basic/.py_c128/lib/python3.12/site-packages/verl/workers/fsdp_workers.py
的修改
'''
...
def make_hook(param_name):
    log_path = "/root/autodl-tmp/LLM_RL_basic/outputs/grad_dtype_log.txt"
    os.makedirs(os.path.dirname(log_path), exist_ok=True)
    def hook_fn(grad):
        ts = datetime.datetime.now().strftime("%F %T")
        with open(log_path, "a", encoding="utf-8") as f:
            f.write(f"[{ts}] {param_name} grad dtype={grad.dtype}, shape={tuple(grad.shape)}\n")
            f.flush()
            os.fsync(f.fileno())
        return grad   # 必须返回 grad，否则梯度会丢失
    return hook_fn
def _trace_log(line: str):
    try:
        os.makedirs("/root/autodl-tmp/LLM_RL_basic/outputs", exist_ok=True)
        with open("/root/autodl-tmp/LLM_RL_basic/outputs/model_from_pretrained.txt",
...
def _mk_once_forward_hook():
    ...
    def hook(mod, inp, out):
        ...
        try:
            x = inp[0]
            _trace_log(f"[FW-HOOK] {mod.__class__.__name__} input={x.dtype}, output={getattr(out, 'dtype', None)}")
        except Exception as e:
            _trace_log(f"[FW-HOOK] error: {e}")
    return hook
def _attach_one_linear_hook(root_module):
    # 找到第一个 Linear（或 QKV/GEMM 模块）做代表
    import torch.nn as nn
    for m in root_module.modules():
        if isinstance(m, nn.Linear):
            m.register_forward_hook(_mk_once_forward_hook())
            break
def _attach_grad_hooks_some_params(root_module, max_count=4):
    cnt = 0
    for n, p in root_module.named_parameters():
        if p.requires_grad:
            p.register_hook(make_hook(n))
            cnt += 1
            if cnt >= max_count:
                break
...
try:
    ...
    # Guard: avoid double patch if this module is imported multiple times in worker forks
    if not getattr(AutoModelForCausalLM, "_from_pretrained_traced", False):
        ...
        def _patched_from_pretrained(cls, *args, **kwargs):
            ...
            if len(args) >= 1:
                ...
                path = kwargs.get("pretrained_model_name_or_path")

            k = kwargs  # shorthand
            info = {
                "path": path,
                "keys": list(k.keys()),
                "torch_dtype": str(k.get("torch_dtype", None)),
                "attn_impl": k.get("attn_implementation", None),
                "trust_remote_code": k.get("trust_remote_code", None),
            }

            # Also include rank/env for context
            rank = os.environ.get("RANK")
            lrank = os.environ.get("LOCAL_RANK")
            world = os.environ.get("WORLD_SIZE")

            _trace_log(
                f"AutoModelForCausalLM.from_pretrained("
                f"path={info['path']}, torch_dtype={info['torch_dtype']}, "
                f"attn_impl={info['attn_impl']}, trust_remote_code={info['trust_remote_code']}, "
                f"rank={rank}, local_rank={lrank}, world_size={world}, "
                f"kwargs_keys={info['keys']})"
            )

            # Call original classmethod
            return _ORIG_FROM_PRETRAINED.__func__(cls, *args, **kwargs)

        # Rebind as classmethod and mark as patched
        AutoModelForCausalLM.from_pretrained = classmethod(_patched_from_pretrained)
        AutoModelForCausalLM._from_pretrained_traced = True

except Exception:
    # Absolutely do not break worker import because of tracing
    pass
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        with init_context(), warnings.catch_warnings():
            ...
            try:
                attn_mod = actor_module.model.layers[0].self_attn
                _trace_log(f"actor attn class={attn_mod.__class__.__name__}")
                _trace_log(f"actor attn impl = {getattr(attn_mod, '_attn_implementation', None)}")
            except Exception as e:
                _trace_log(f"inspect attn class failed: {e}")
        ...
        # A-2: 前向 dtype 取样（只需要一次）
        _attach_one_linear_hook(actor_module_fsdp)
        # B-1: 只对 actor 侧挂梯度 hook（ref 计算 logprob 不需要反向）
        if role == "actor":
            _attach_grad_hooks_some_params(actor_module_fsdp, max_count=4)
        ...
        with self.ulysses_sharding_manager:
            ...
            for i, group in enumerate(self.actor_optimizer.param_groups):
                for p in group['params']:
                    ...
                    if 'exp_avg' in state:
                        _trace_log(f"param {tuple(p.shape)}, exp_avg={state['exp_avg'].dtype}, exp_avg_sq={state['exp_avg_sq'].dtype}")
                        break  # 只看一个就够
                break
